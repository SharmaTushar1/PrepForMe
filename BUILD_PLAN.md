...
- **Database** — 16-table Postgres schema, RLS on every table, Data API grants applied, triggers (seed profile/settings on signup, stage-event history, `applied_at` stamp, `updated_at`). The first 14 tables are deployed to Supabase Mumbai and verified end-to-end; `0004`'s two resume tables and its Storage bucket are applied and checked on the local stack, and not yet applied to the hosted project.
...
- **The analyzer is switched on in local development, and nowhere else.** `.env.development.local` sets `VITE_AI_PROVIDER=edge` and the Anthropic key is in `supabase/.env.local`, so with the function served, pressing "Analyze resume" under `npm run dev` bills a real key against an account with no cap on it. Nothing spends on its own — the press is always explicit, and storing a file reads nothing — but "a dev machine can't spend money by accident" stopped being true of this machine, and has since stopped being hypothetical too. Nothing is on anywhere else: `0004` isn't applied to the hosted project, the Edge Function isn't deployed, its secret isn't set, and any build without `VITE_AI_PROVIDER` gets the local provider, which returns a labelled sample.
